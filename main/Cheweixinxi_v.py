...
def cheweixinxi_resetPass(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        try:
            __loginUserColumn__= cheweixinxi.__loginUserColumn__
        except:
        ...
        records=cheweixinxi.getbyparams(cheweixinxi, cheweixinxi, recordsParam)
        if len(records)<1:
            msg['code'] = 400
            msg['msg'] = '用户不存在'
            return JsonResponse(msg)

        eval('''cheweixinxi.objects.filter({}='{}').update({}='{}')'''.format(username_str,username,password_str,init_pwd))
...
def cheweixinxi_page(request):
    ...
    if request.method in ["POST", "GET"]:
        ...
        # 当前表也是有管理员权限的表
        if  __isAdmin__ == "是" and 'cheweixinxi' != 'forum':
            if req_dict.get("userid") and 'cheweixinxi' != 'chat':
                del req_dict["userid"]

        else:
            #非管理员权限的表,判断当前表字段名是否有userid
            if tablename!="users" and 'cheweixinxi'[:7]!='discuss'and "userid" in cheweixinxi.getallcolumn(cheweixinxi,cheweixinxi):
                req_dict["userid"] = request.session.get("params").get("id")
        ...
        msg['data']['pageSize']  =cheweixinxi.page(cheweixinxi, cheweixinxi, req_dict, request, q)
...
主要信息列表（如商品列表，新闻列表）中使用，显示最近点击的或最新添加的5条记录就行
    ...
    if request.method in ["POST", "GET"]:
        ...
        elif "browseduration"  in cheweixinxi.getallcolumn(cheweixinxi,cheweixinxi):
...
def cheweixinxi_info(request,id_):
    ...
    if request.method in ["POST", "GET"]:
        ...
        if __browseClick__=="是"  and  "clicknum"  in cheweixinxi.getallcolumn(cheweixinxi,cheweixinxi):
            try:
...
def cheweixinxi_update(request):
    ...
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": mes.normal_code, "data": {}}
        req_dict = request.session.get("req_dict")
        if req_dict.get("mima") and "mima" not in cheweixinxi.getallcolumn(cheweixinxi,cheweixinxi) :
            del req_dict["mima"]
        try:
...
def cheweixinxi_importExcel(request):
    if request.method in ["POST", "GET"]:
        ...
        if file_type in ['xlsx', 'xls']:
            ...
            try:
                for row in range(1, rows):
                    ...
                    req_dict = {}
                    cheweixinxi.createbyreq(cheweixinxi, cheweixinxi, req_dict)
...
def cheweixinxi_sendemail(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
...
#收藏协同算法
def cheweixinxi_autoSort2(request):
    if request.method in ["POST", "GET"]:
        req_dict = request.session.get("req_dict")
        cursor = connection.cursor()
        ...
        user_ratings={}
        try:
            cursor.execute("select * from storeup where type = 1 and tablename = 'cheweixinxi' order by addtime desc")
        ...
        if where:
            sql = f'''SELECT * FROM (SELECT * FROM cheweixinxi WHERE {where}) AS table1 WHERE id IN ('{"','".join(sorted_recommended_goods)}') union all SELECT * FROM (SELECT * FROM cheweixinxi WHERE {where}) AS table1 WHERE id NOT IN ('{"','".join(sorted_recommended_goods)}')'''
        else:
            sql = f'''select * from cheweixinxi where id in ('{"','".join(sorted_recommended_goods)}') union all select * from cheweixinxi where id not in('{"','".join(sorted_recommended_goods)}')'''
        cursor.execute(sql)
        desc = cursor.description
        data_dict = [dict(zip([col[0] for col in desc], row)) for row in cursor.fetchall()]
...
def cheweixinxi_value(request, xColumnName, yColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        ...
        sql = ''
        if timeStatType == '日':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM cheweixinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d') LIMIT 10".format(xColumnName, yColumnName, where, '%Y-%m-%d')

        if timeStatType == '月':
            sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM cheweixinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m') LIMIT 10".format(xColumnName, yColumnName, where, '%Y-%m')

        if timeStatType == '年':
            sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM cheweixinxi {2} GROUP BY DATE_FORMAT({0}, '%Y') LIMIT 10".format(xColumnName, yColumnName, where, '%Y')
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
...
def cheweixinxi_o_value(request, xColumnName, yColumnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        
        where = ' where 1 = 1 '
        
        sql = "SELECT {0}, sum({1}) AS total FROM cheweixinxi {2} GROUP BY {0} LIMIT 10".format(xColumnName, yColumnName, where)
        L = []
        cursor = connection.cursor()
        cursor.execute(sql)
        desc = cursor.description
        ...
        # （按值统计）时间统计类型(多)
def cheweixinxi_valueMul(request, xColumnName, timeStatType):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = ''
            if timeStatType == '日':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m-%d') {0}, sum({1}) total FROM cheweixinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m-%d') LIMIT 10".format(xColumnName, item, where, '%Y-%m-%d')

            if timeStatType == '月':
                sql = "SELECT DATE_FORMAT({0}, '%Y-%m') {0}, sum({1}) total FROM cheweixinxi {2} GROUP BY DATE_FORMAT({0}, '%Y-%m') LIMIT 10".format(xColumnName, item, where, '%Y-%m')

            if timeStatType == '年':
                sql = "SELECT DATE_FORMAT({0}, '%Y') {0}, sum({1}) total FROM cheweixinxi {2} GROUP BY DATE_FORMAT({0}, '%Y') LIMIT 10".format(xColumnName, item, where, '%Y')
...
#（按值统计(多)）
def cheweixinxi_o_valueMul(request, xColumnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": []}
        ...
        for item in req_dict['yColumnNameMul'].split(','):
            sql = "SELECT {0}, sum({1}) AS total FROM cheweixinxi {2} GROUP BY {0} LIMIT 10".format(xColumnName, item, where)
            L = []
            cursor = connection.cursor()
            cursor.execute(sql)
            desc = cursor.description
...
def cheweixinxi_group(request, columnName):
    if request.method in ["POST", "GET"]:
        msg = {"code": normal_code, "msg": "成功", "data": {}}
        
        where = ' where 1 = 1 '

        sql = "SELECT COUNT(*) AS total, " + columnName + " FROM cheweixinxi " + where + " GROUP BY " + columnName + " LIMIT 10" 
